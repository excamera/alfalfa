...
int main( int argc, char *argv[] )
{
  try {
	...
	parsed_frame.decode( source_decoder_state.segmentation, source_raster_preloop );
	source_raster.get().copy_from( source_raster_preloop );
	...
	parsed_frame.decode( source_decoder_state.segmentation,
			     source_references, source_raster_preloop );
	source_raster.get().copy_from( source_raster_preloop );
	parsed_frame.loopfilter( source_decoder_state.segmentation, source_decoder_state.filter_adjustments, source_raster );
	parsed_frame.copy_to( source_raster, source_references );
      }
	...
	parsed_frame.decode( target_decoder_state.segmentation, target_raster_preloop );
	target_raster.get().copy_from( target_raster_preloop );
	...
	if ( whole_frame.key_frame() ) {
	  ...
	  parsed_frame.decode( test_decoder_state.segmentation, test_raster );
